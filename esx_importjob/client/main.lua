  ...
  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'import_actions', {
      ...
      if data.current.value == 'menu_crafting' then
          ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'menu_crafting', {
                ...
                TriggerServerEvent('esx_importjob:craftingCoktails', data2.current.value)
  ...
  ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_import', _U('billing'), amount)
        end
      else
...
function OpenGetFridgeStocksMenu()
  ESX.TriggerServerCallback('esx_importjob:getFridgeStockItems', function(items)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fridge_menu', {
        ...
        ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'fridge_menu_get_item_count', {
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_importjob:getFridgeStockItem', itemName, count)
            end
          end, function(data2, menu2)
            menu2.close()
          end)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fridge_menu', {
        ...
        ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'fridge_menu_put_item_count', {
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_importjob:putFridgeStockItems', itemName, count)
            end
          end, function(data2, menu2)
            menu2.close()
          end)
...
function OpenGetWeaponMenu()
    ESX.TriggerServerCallback('esx_importjob:getVaultWeapons', function(weapons)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vault_get_weapon', {
        ...
        ESX.TriggerServerCallback('esx_importjob:removeVaultWeapon', function()
          OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
  ...
  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vault_put_weapon', {
      ...
      ESX.TriggerServerCallback('esx_importjob:addVaultWeapon', function()
        OpenPutWeaponMenu()
      end, data.current.value)
    end, function(data, menu)
      menu.close()
...
function OpenShopMenu(zone)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'import_shop', {
            title    = _U('shop'),
            elements = elements
        }, function(data, menu)
            TriggerServerEvent('esx_importjob:buyItem', data.current.value, data.current.price, data.current.realLabel)
        end, function(data, menu)
            menu.close()
        end)
end
...
-----------------------
----- TELEPORTERS -----
AddEventHandler('esx_importjob:teleportMarkers', function(position)
  ...
  while true do
    ...
    if IsJobTrue() then
        ...
        if IsControlJustReleased(0, 38) and isInPublicMarker then
          TriggerEvent('esx_importjob:teleportMarkers', position)
        end
